...
import com.github.skac112.fractalizer.nodes.composition._
...
//  }
  ...
  def |(other: Node) = com.github.skac112.fractalizer.nodes.composition.Split(this, other)

  def |*(times: Int) = com.github.skac112.fractalizer.nodes.composition.SplitTimes(this, times)
  ...
  /**
    ...
    * @return
    */
  def withDraw(drawFun: EnsFun): Node = fromFun(makeNodeFun(drawFun, this.procFun _),
    internalArity)

  def withDraw(drawNode: Node): Node = withDraw(drawNode.drawFun _)

  def withProc(procFun: EnsFun, internalArity: Int = 1): Node =
    fromFun(makeNodeFun(this.drawFun _, procFun), internalArity)
